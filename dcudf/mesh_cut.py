    ...
    if isinstance(scene_or_mesh, trimesh.Scene):
        if len(scene_or_mesh.geometry) == 0:
            ...
            mesh = trimesh.util.concatenate(
                tuple(
                    trimesh.Trimesh(vertices=g.vertices, faces=g.faces, process=False)
...
def mesh_cut(mesh, region_rate=20, exp_weight=200):
    ...
    cc = trimesh.graph.connected_components(
        mesh.face_adjacency, min_len=len(mesh.vertices) // 5
    )
            ...
            if idx not in seed_neighbor:
                ...
                for e in edges:
                    if e[0] == seed:
                        ...
                        g.add_edge(
                            nodes[ori_g[int(e[0])]], nodes[ori_g[int(e[1])]], e[2], e[2]
                        )
                # run mini cut
                flow = g.maxflow()
                # gather two parts
                ...
                rate = abs(len(partition[0]) - len(partition[1])) / (
                ...
                seed_list = set(
                    np.array(
                        list(partition[0].union(seed_neighbor)), dtype=int
                    ).tolist()
                )
                sink_list = set(
                    np.array(
                        list(partition[1].union(sink_neighbor)), dtype=int
                    ).tolist()
                )

                out_mesh_1 = trimesh.base.Trimesh(
                    vertices=mesh.vertices,
                    faces=mesh.faces[list(seed_list)],
                    process=False,
                )
                out_mesh_2 = trimesh.base.Trimesh(
                    vertices=mesh.vertices,
                    faces=mesh.faces[list(sink_list)],
                    process=False,
                )
    ...
    with open("cut_time.csv", "w") as f:
        for path in os.listdir(root):
            ...
            mesh_name = os.path.join(
                root, path, "mesh", "{}_399_Optimize_0.005.ply".format(path)
            )
            mesh = as_mesh(trimesh.load_mesh(mesh_name, process=False))
            out = mesh_cut(mesh)
            if out is not None:
