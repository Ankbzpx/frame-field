...
def y_4_1(x, y, z):
    return (3 / 4) * np.sqrt(
        5 / (2 * np.pi)) * y * (7 * z**3 - 3 * z * r_2(x, y, z))


def y_40(x, y, z):
    return (3 / 16) * np.sqrt(
        1 / np.pi) * (35 * z**4 - 30 * z**2 * r_2(x, y, z) + 3 * r_4(x, y, z))


def y_41(x, y, z):
    return (3 / 4) * np.sqrt(
        5 / (2 * np.pi)) * x * (7 * z**3 - 3 * z * r_2(x, y, z))


def y_42(x, y, z):
    return (3 / 8) * np.sqrt(
        5 / np.pi) * (x**2 - y**2) * (7 * z**2 - r_2(x, y, z))
...
def y_44(x, y, z):
    return (3 / 16) * np.sqrt(35 / np.pi) * (x**2 * (x**2 - 3 * y**2) - y**2 *
                                             (3 * x**2 - y**2))
...
def R3_to_sh4_zonal(R3):
    return zonal_to_octa_scale * zonal_sh4_coeffs(torch.transpose(R3, -1,
                                                                  -2)).sum(1)
