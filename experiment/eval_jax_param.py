...
from common import filter_components, ps_register_basis, voxel_tet_from_grid_scale
...
def extract_dc(save_path, f, f_grad, f_map, grid_res, grid_min=-1.0, grid_max=1.0):
    min_corner = np.float64([grid_min, grid_min, grid_min])
    max_corner = np.float64([grid_max, grid_max, grid_max])

    V, F = frame_field_utils.dual_contouring_serial(
...
def extract_mc(save_path, f, f_map, grid_res, grid_min=-1.0, grid_max=1.0):
    V, F, VN = extract_surface(
        vmap(f), grid_res=grid_res, grid_min=grid_min, grid_max=grid_max
    )
...
def eval(
    ...
    if inverse:
        ...
        if dc:
            V_param, F_param = extract_dc(
                f"{out_dir}/{cfg.name}_param_dc.obj",
                infer_sdf_param,
                infer_sdf_grad_param,
                infer_mapping,
                grid_res,
                grid_min,
                grid_max,
            ...
            V_param, F_param = extract_mc(
                f"{out_dir}/{cfg.name}_param_mc.obj",
                infer_sdf_param,
                infer_mapping,
                grid_res,
                grid_min,
