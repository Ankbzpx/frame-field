...
if __name__ == "__main__":
    ...
    rotvec_0 = np.random.randn(
        3,
    )
    rotvec_1 = np.random.randn(
        3,
    )

    rotvecs_interp = (
        np.linspace(1, 0, n_step)[:, None] * rotvec_0[None, :]
        + np.linspace(0, 1, n_step)[:, None] * rotvec_1[None, :]
    )

    V_0 = np.stack([np.zeros(n_step), np.linspace(-1, 1, n_step), np.zeros(n_step)], -1)

    V_0_vis, F_0_vis = vis_oct_field(vmap(rotvec_to_R3)(rotvecs_interp), V_0, 0.1)

    sh4_0 = rotvec_to_sh4(rotvec_0)
    sh4_1 = rotvec_to_sh4(rotvec_1)

    sh4_interp = (
        np.linspace(1, 0, n_step)[:, None] * sh4_0[None, :]
        + np.linspace(0, 1, n_step)[:, None] * sh4_1[None, :]
    )

    V_1 = np.stack([np.ones(n_step), np.linspace(-1, 1, n_step), np.zeros(n_step)], -1)
    ...
    V_2 = np.stack(
        [2 * np.ones(n_step), np.linspace(-1, 1, n_step), np.zeros(n_step)], -1
    )
    ...
    V_3 = np.stack(
        [3 * np.ones(n_step), np.linspace(-1, 1, n_step), np.zeros(n_step)], -1
    )
