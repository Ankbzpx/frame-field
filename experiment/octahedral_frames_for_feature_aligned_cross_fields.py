...
if __name__ == "__main__":
    ...
    W = scipy.sparse.coo_array(
        (np.ones(7), (np.arange(7), 1 + np.arange(7))), shape=(7, 9)
    ).todense()
    ...
    if norm != "2":
        ...
        def loss_func(x):
            ...
            loss_smooth = jnp.linalg.norm(E_weight * edge_energy, ord=p) / len(
                edge_energy
            ) ** (1 / p)
            # Inherently soft constraints...
            loss_align = jnp.linalg.norm(
                jnp.einsum("bji,bi->bj", As, x) - b[None, :], ord=2, axis=1
            ).mean()
        ...
        while True:
            x = solve_least_square(L_unroll, np.zeros(9 * NV), B, bc, boundary_weight)
            x = x.reshape(NV, 9)
                    ...
                    scipy.sparse.coo_array(
                        (
                            ...
                            (
                                np.arange(9 * N_rerun),
                                (
                                    (9 * rerun_vid)[..., None] + np.arange(9)[None, ...]
                                ).reshape(-1),
                            ),
                        ),
                        shape=(9 * N_rerun, 9 * NV),
                    ).tocsc(),
                ]
            )
