...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "hex_path",
        type=str,
        help="Path to tetrahedral mesh with seamless hexahedral parameterization (optionally with walls).",
    )
    ...
    F_param = (
        4 * np.arange(len(hex_mesh.uvws))[:, None, None] + F_param[None, ...]
    ).reshape(-1, 3)
    ...
    if len(hex_mesh.wall_facets) > 0:
        ps.register_surface_mesh("wall", hex_mesh.vertices, hex_mesh.wall_facets)
    ps.show()
