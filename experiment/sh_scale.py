...
if __name__ == "__main__":
    ...
    sh4_4_xy = (
        zonal_to_octa_scale
        * (
            zonal_sh4_coeffs(np.array([1, 0, 0]))
            + zonal_sh4_coeffs(np.array([0, 1, 0]))
        )[4]
    )
    ...
    err = np.abs(
        sh4
        @ proj_sh4_sdp(sh4_z_scale).reshape(
            -1,
        )
        - 1
    )
    ...
    assert np.isclose(
        sh4_z_scale[4], z_scale**4 * (np.sqrt(7 / 12) - sh4_4_xy) + sh4_4_xy
    )
    ic(sh4_z_scale, err)
    ...
    err = np.abs(
        sh4
        @ proj_sh4_sdp(sh4_xy_scale).reshape(
            -1,
        )
        - 1
    )
    ...
    sh4_4_xy_scale = (
        ...
        * (
            ...
            + zonal_sh4_coeffs(np.array([0, xy_scale, 0]))
        )[4]
    )
    ...
    assert np.isclose(
        np.sqrt(sh4_xy_scale[0] ** 2 + sh4_xy_scale[8] ** 2),
    ...
    np.isclose(np.delete(project_z(sh4, xy_scale**4), 4), np.delete(sh4_xy_scale, 4))
    ic(sh4_xy_scale, err)
