...
from common import normalize, Timer, vis_oct_field
from config import Config, LossConfig
from config_utils import config_latent, config_model, config_toy_training_data
...
def train(cfg: Config, model: model_jax.MLP, data):
    ...
    def loss_func(
        ...
        if loss_cfg.smooth > 0:
            jac_on, ((pred_on_sur_sdf, aux_on), pred_normals_on_sur) = (
                model.call_jac_param(samples_on_sur, latent, param_func)
            )
            jac_off, ((pred_off_sur_sdf, aux_off), pred_normals_off_sur) = (
                model.call_jac_param(samples_off_sur, latent, param_func)
            )
        else:
...
def eval(
    ...
    if vis:
        ...
        ps.register_surface_mesh("Oct frames interpolation", V_vis_interp, F_vis_interp)
        ps.show()
    ...
    igl.write_triangle_mesh(f"{out_dir}/{cfg.name}_sup.obj", V_vis_sup, F_vis_sup)
    igl.write_triangle_mesh(
        f"{out_dir}/{cfg.name}_interp.obj", V_vis_interp, F_vis_interp
    )
    ...
    for gap in [4]:
        for theta in [90]:
            ...
            if args.eval:
                ...
                data = config_toy_training_data(
                    cfg, samples_sup, samples_vn_sup, latents
                )
