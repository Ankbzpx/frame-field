...
def train(cfg: Config, model: model_jax.MLP, data):
    ...
    def loss_func(
        ...
        if udf:
            ...
            sh4_jac_on, ((udf_on, aux_on), pred_normals_on_sur) = model.call_jac_param(
                samples_on_sur, lambda x: x
            )
            sh4_jac_close, ((udf_close, aux_close), pred_normals_close_sur) = (
                model.call_jac_param(samples_close_sur, lambda x: x)
            )
            ...
            sh4_jac = jnp.vstack([sh4_jac_on, sh4_jac_close])
