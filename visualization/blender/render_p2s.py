...
def render_pointcloud(save_path, pc_path, location, rotation, scale, radius=3e-3):
    load_template()
...
def render_mesh(save_path, mesh_path, location, rotation, scale, face_normals=False):
    load_template()
    ...
    for dataset in ["abc", "thingi10k"]:
        ...
        for noise_level in ["1e-2", "2e-3"]:
            ...
            for model_name in model_list:
                ...
                save_folder = os.path.join(root_folder, "p2s_render", tag, model_name)
                ...
                gt_path = os.path.join(
                    root_folder, "p2s", dataset, "gt", f"{model_name}.ply"
                )
                ...
                input_path = os.path.join(
                    root_folder, "p2s", dataset, noise_level, f"{model_name}.ply"
                )
                input_save_path = os.path.join(save_folder, "input.png")
                if render_model or not os.path.exists(input_save_path):
                    try:
                        render_pointcloud(
                            input_save_path, input_path, location, rotation, scale
                        )
                    except subprocess.CalledProcessError:
                        failure_case_list.append(f"{model_name}_{tag}_input")

                for method in methods:
                    render_method = render_model or (method in re_render_method_list)

                    model_path = glob(
                        os.path.join(
                            root_folder, "octa_results", method, tag, f"{model_name}.*"
                        )
                    )[0]
                    ...
                    if render_method or not os.path.exists(model_save_path):
                        try:
                            ...
                            failure_case_list.append(f"{model_name}_{tag}_{method}")
